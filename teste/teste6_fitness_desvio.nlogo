globals [ max-sheep season day years difbytime total-energy-wolf total-energy-sheep avg-energy-wolf avg-energy-sheep list-energy-sheep list-energy-wolf act-pct-wolf act-pct-sheep prev-pct-wolf prev-pct-sheep pct-wolf pct-sheep pct-wolf-sheep list-pct-wolf-sheep]  ; don't let sheep population grow too large
...
to setup
  ...
  set total-energy-wolf 0
  set total-energy-sheep 0

  set list-energy-sheep []
  set list-energy-wolf []

  set pct-wolf-sheep 0
  set list-pct-wolf-sheep []

  set prev-pct-wolf 0
  set act-pct-wolf 0

  set prev-pct-sheep 0
  set act-pct-sheep 0
...
to go
  ...
  [
    set ticksnozero ticks
  ]

  set difbytime (count sheep) - (count wolves) / ticksnozero
  ...
  [
    ifelse (day >= 91 AND day < 181)
    [
      set season "fall"
    ]
    [
      ifelse (day >= 181 AND day < 271)
      [
        set season "winter"
      ]
      [
  ...
  ;;Spring tem 1 dia a mais


  if not any? sheep [ user-message "Fim das ovelhas" stop ]
  if not any? wolves [ user-message "Fim dos lobos" stop ]
  ...
  ask sheep [
    ...
    if random 100 < sudden-death-chance
    [
      ;print ("morte súbita")
      die
    ]

    set total-energy-sheep (total-energy-sheep + energy)
    set avg-energy-sheep (total-energy-sheep / count sheep)
  ]
  ask wolves [
    move
    ...
    set total-energy-wolf (total-energy-wolf + energy)

    set avg-energy-wolf (total-energy-wolf / count wolves)
  ...
  set act-pct-wolf avg-energy-wolf

  set act-pct-sheep avg-energy-sheep
...
;    print prev-pct-wolf
    ...
    if (prev-pct-sheep = 0)
    [
      set prev-pct-sheep 1
    ]

    set pct-wolf (precision ( abs( 100 - ( ( 100 * act-pct-wolf ) / prev-pct-wolf ) ) ) 3)
    set pct-sheep (precision ( abs( 100 - ( ( 100 * act-pct-sheep ) / prev-pct-sheep ) ) ) 3)
    ;print pct-wolf
  ]

  set prev-pct-wolf avg-energy-wolf

  set prev-pct-sheep avg-energy-sheep

  set pct-wolf-sheep (pct-wolf + pct-sheep)

  set list-pct-wolf-sheep lput pct-wolf-sheep list-pct-wolf-sheep




;  if(length list-energy-wolf = 5)
;  [
;    set list-energy-wolf but-first list-energy-wolf
;  ]
;
;  set list-energy-wolf lput avg-energy-wolf list-energy-wolf
;
;  print (list-energy-wolf)
...
to reproduce-sheep  ; sheep procedure
  if random-float 100 < sheep-reproduce [  ; throw "dice" to see if you will reproduce
    ...
    hatch 1 [ rt random-float 360 fd 1 set age (365 * sheep-max-age)]   ; hatch an offspring and move it forward 1 step
  ]
end
...
to reproduce-wolves  ; wolf procedure
  if random-float 100 < wolf-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 set age (365 * wolf-max-age)]  ; hatch an offspring and move it forward 1 step
  ]
end
...
to random-temp-by-season
  ...
  [
    ...
    [
      set min-temp min-temp-summer
      set max-temp max-temp-summer
    ]
    [
      ifelse(season = "fall")
      [
        set min-temp min-temp-fall
        set max-temp max-temp-fall
      ]
      [
        ifelse(season = "winter")
        [
          set min-temp min-temp-winter
          set max-temp max-temp-winter
        ]
        [
          if(season = "spring")
          [
            set min-temp min-temp-spring
            set max-temp max-temp-spring
          ]
        ]
      ]
    ]
    ...
    let abd random (max-temp - min-temp + 1) + min-temp

    ;print abd

    if (abd >= min-ideal-temp-grass AND abd <= max-ideal-temp-grass)
    [
      ask patches [
        let division (countdown / 2)
        ifelse (division <= 1)
        [
          set countdown 0
        ]
        [
          set countdown division
        ]
        ;      set pcolor one-of [ green brown ]
        ;      ifelse pcolor = green
        ;        [ set countdown grass-regrowth-time ]
    ...
    ;  [
    ;    print abd
    ;    set abd random (max-temp-summer - min-temp-summer + 1) + min-temp-summer
    ;  ]
...
to load-config-from-file
  ...
  [
    ...
    ifelse(item0 = "max-temp-fall")
    [
      set max-temp-fall item1
    ]
    [
      ifelse(item0 = "wolf-gain-from-food")
      [
        set wolf-gain-from-food item1
      ]
      [
        ifelse(item0 = "max-temp-spring")
        [
          set max-temp-spring item1
        ]
        [
          ifelse(item0 = "show-energy?")
          [
            set show-energy? item1
          ]
          [
            ifelse(item0 = "wolf-reproduce")
            [
              set wolf-reproduce item1
            ]
            [
              ifelse(item0 = "initial-number-wolves")
              [
                set initial-number-wolves item1
              ]
              [
                ifelse(item0 = "initial-number-sheep")
                [
                  set initial-number-sheep item1
                ]
                [
                  ifelse(item0 = "model-version")
                  [
                    set model-version item1
                  ]
                  [
                    ifelse(item0 = "sheep-gain-from-food")
                    [
                      set sheep-gain-from-food item1
                    ]
                    [
                      ifelse(item0 = "min-temp-summer")
                      [
                        set min-temp-summer item1
                      ]
                      [
                        ifelse(item0 = "grass-regrowth-time")
                        [
                          set grass-regrowth-time item1
                        ]
                        [
                          ifelse(item0 = "min-temp-winter")
                          [
                            set min-temp-winter item1
                          ]
                          [
                            ifelse(item0 = "min-temp-fall")
                            [
                              set min-temp-fall item1
                            ]
                            [
                              ifelse(item0 = "max-temp-summer")
                              [
                                set max-temp-summer item1
                              ]
                              [
                                ifelse(item0 = "max-temp-winter")
                                [
                                  set max-temp-winter item1
                                ]
                                [
                                  ifelse(item0 = "enable-seasons")
                                  [
                                    set enable-seasons item1
                                  ]
                                  [
                                    ifelse(item0 = "min-temp-spring")
                                    [
                                      set min-temp-spring item1
                                    ]
                                    [
                                      if(item0 = "sheep-reproduce")
                                      [
                                        set sheep-reproduce item1
                                      ]
                                    ]
                                  ]
                                ]
...
to export-ticks
  let path "/teste/export-ticks.txt"
  ;Pra pensar, se limpa ou não
  carefully [ file-delete path ] [ ]
  file-open path
  ;print ("ta entrando")
  ;file-write ["ticks:" ticks]
  ;file-write ["days:" day]
  file-write ((standard-deviation list-pct-wolf-sheep) + (36500 - ticks))
  file-close

  ;print(word "" "End of simulation. Ticks:" ticks ". Information saved on " path)
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot (abs (count sheep) - (count wolves))"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot avg-energy-wolf"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot avg-energy-sheep"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot pct-wolf"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot pct-sheep"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot (pct-wolf + pct-sheep)"
...
<experiments>
  <experiment name="experiment1" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="max-temp-fall">
      <value value="20"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wolf-gain-from-food">
      <value value="20"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-temp-spring">
      <value value="40"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="min-temp-summer">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="grass-regrowth-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="min-temp-winter">
      <value value="21"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="min-temp-fall">
      <value value="11"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-temp-summer">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-temp-winter">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="enable-seasons">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="min-temp-spring">
      <value value="31"/>
    </enumeratedValueSet>
